...
from utils.loss import (NCA, BCESigmoid, BCEWithLogitsLossWithIgnoreIndex,
                        ...
                        soft_crossentropy)
from utils.entropy import EntropyWeights
        ...
        for cur_step, (images, labels) in enumerate(train_loader):
            ...
            if self.pseudo_soft is not None:
                ...
                if self.ce_on_pseudo and self.step > 0:
                    ...
                    # if self.step > 0 and hasattr(self, 'nb_current_classes'):
                    #     sample_weights = self.entropy_weights.get_weight(labels, outputs_old)
                    #     loss = cross_entropy(outputs,labels,weight=sample_weights,ignore_index=255)
                    # else:
    ...
    def validate(self, loader, metrics, ret_samples_ids=None, logger=None, end_task=False):
        ...
        with torch.no_grad():
            for i, (images, labels) in enumerate(loader):
                ...
                if self.step > 0 and hasattr(self, 'model_old') and self.model_old is not None:
                    outputs_old, features_old = self.model_old(images, ret_intermediate=True)
                    mask_background = labels < self.old_classes
                    labels_old = labels.clone().detach()
                    labels_old[labels_old == 255] = 0
                    labels_old[mask_background] = outputs_old.argmax(dim=1)[mask_background]
                    weight, num_weight, entropy_weight, seg_entropy = self.entropy_weights.show_weight(labels_old, outputs_old)
                ...
                if self.step > 0 and hasattr(self, 'model_old') and self.model_old is not None:
                    pass
                    # self.entropy_weights.entropy_statistics(labels, prediction, seg_entropy, self.step)
                # if ret_samples_ids is not None and i in ret_samples_ids:  # get samples
                #     ret_samples.append((images[0].detach().cpu().numpy(), labels[0], prediction[0], labels_old[0]))
                if ret_samples_ids is not None and i in ret_samples_ids:  # get samples
                    if self.step > 0 and hasattr(self, 'model_old'):
                        ret_samples.append((images_ori[0].detach().cpu().numpy(), labels[0], prediction[0], labels_old[0].detach().cpu().numpy(), seg_entropy[0].detach().cpu().numpy(), entropy_weight[0].detach().cpu().numpy(), weight[0].detach().cpu().numpy(), num_weight[0].detach().cpu().numpy()))
                    else:
                        ret_samples.append((images_ori[0].detach().cpu().numpy(), labels[0], prediction[0], labels[0], None, None, None, None))
