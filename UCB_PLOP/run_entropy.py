...
from train_entropy import Trainer
from utils.logger import Logger
import matplotlib.pyplot as plt
import datetime
import cv2

def show_weight(weight, ori_img, min_max = True):
    cmap = plt.get_cmap('jet')
    cmap = cmap.reversed()
    if min_max:
        weight = (weight - weight.min()) / (weight.max() - weight.min())
    colored_map = cmap(weight)[:, :, :3]

    alpha = 0.5
    image = (ori_img * (1 - alpha) + colored_map * 255 * alpha)
    return image
...
def run_step(opts, world_size, rank, device):
    ...
    logdir_full = os.path.join(logdir_full, now_time)
    ...
    if TRAIN:
        ...
        for cur_epoch in range(opts.epochs):
            ...
            if (cur_epoch + 1) % opts.val_interval == 0:
                ...
                for k, (img, target, lbl, labels_old, entropy, entropy_weight, weight, num_weight) in enumerate(ret_samples):
                    ...
                    labels_old = label2color(labels_old).astype(np.uint8)

                    if opts.step == 0:
                        concat_img = np.concatenate((img, target, lbl, labels_old), axis=1)  # concat along width
                    else:
                        weight = show_weight(weight, img.copy())
                        entropy = show_weight(entropy, img.copy())
                        entropy_weight=  show_weight(entropy_weight, img.copy())
                        num_weight = show_weight(num_weight, img.copy())
                        concat_img = np.concatenate((img, target, lbl, labels_old, weight, entropy, entropy_weight, num_weight), axis=1)
                    concat_img = np.clip(concat_img, 0, 255).astype(np.uint8)
                    concat_img = cv2.cvtColor(concat_img, cv2.COLOR_BGR2RGB)
                    # cv2.imwrite('contact.jpg', concat_img)
