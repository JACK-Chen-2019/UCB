...
def main(opts):
    ...
    val_interval = max(100, total_itrs // 2)
    # val_interval = 30
    ...
    if opts.curr_step > 0:
        entropy_weights = EntropyWeights(curr_idx[1], curr_idx[0])
    ...
    while cur_itrs < total_itrs:
        ...
        """ data load """
        try:
            images, labels, sal_maps, images_name = train_iter.__next__()
        except:
        ...
        if opts.curr_step > 0 and opts.mem_size > 0:
            try:
                m_images, m_labels, m_sal_maps, _ = mem_iter.__next__()
            except:
        ...
        with torch.cuda.amp.autocast(enabled=opts.amp):
            ...
            if opts.pseudo and opts.curr_step > 0:
                ...
                pseudo_labels = torch.where((labels <= fg_idx) & (pred_labels > fg_idx), pred_labels, labels)
            ...
            if opts.pseudo and opts.curr_step > 0:
                ...
                lpixel = lpixel * weight
                lproposal = lproposal * weight

            lpixel = lpixel.mean()
            lproposal = lproposal.mean()
        ...
        if val_interval > 0 and (cur_itrs) % val_interval == 0:
            print("validation...")
            model.eval()
            val_score = validate(opts=opts, model=model, loader=test_loader,
                                 device=device, metrics=metrics, alpha=loss_tred_alpha)
            print(val_score)
