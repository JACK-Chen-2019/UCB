...
from deeplab_jittor.backbone import resnet101
from deeplab_jittor.aspp import ASPP
from deeplab_jittor.decoder import MyDecoder
...
def make_model(opts, classes=None):
    if opts.backbone == "resnet101":
        ...
        body = resnet101(output_stride=opts.output_stride)
        head = DeepLabHead(output_stride=opts.output_stride) 
...
class IncrementalSegmentationModule(nn.Module):
    ...
    def __init__(
        ...
        if use_cosine:
            ...
            self.scalar = nn.Parameter(jt.Var(1.)).float()
            #===
        ...
        if len(self.classes) > 1 and opts.warm_up:
            ...
            if opts.two_stage:
                ...
                self.weight_old = nn.Parameter(jt.zeros(self.new_classes, self.old_classes, self.embedding_dim))
                nn.init.kaiming_normal_(self.weight_old, mode='fan_out')
                self.weight_new = nn.Parameter(jt.zeros(self.new_classes, self.old_classes))
                nn.init.kaiming_normal_(self.weight_new, mode='fan_out')
                self.new_bias = nn.Parameter(self.cls[0].bias[0].repeat(self.new_classes))
                self.weight_old_bg = nn.Parameter(jt.ones(1, self.embedding_dim))
                self.weight_new_bg = nn.Parameter(jt.ones(1, 1))
    ...
    def _network(self, x, ret_intermediate=False, only_bg=False):
        ...
        x_b, low_level_feat = self.body(x)
        # ret = self.body(x)
        ...
        x_pl = self.head(x_b, low_level_feat)
        #==========xzy modified end==========
        out = []
        ...
        x_o = jt.contrib.concat(out, dim=1)
    ...
    def avg_weight_output(self, x):
        ...
        for i, mod in enumerate(self.cls[:-1]):
            if i == 0 and (not self.opts.two_stage):
                ...
                new_weight_bg = (self.weight_old_bg) * (mod.weight[0].unsqueeze(0).squeeze(-1).squeeze(-1))
                new_weight_bg = jt.nn.matmul(self.weight_new_bg, new_weight_bg).unsqueeze(-1).unsqueeze(-1)
                task0_weight = jt.contrib.concat([new_weight_bg, mod.weight[1:]])
                out.append(jt.nn.conv2d(x_pl, task0_weight, mod.bias)) 
            else :
                out.append(mod(x_pl))

        imprinting_w = jt.contrib.concat([x.weight for x in self.cls[:-1]], dim=0).squeeze(-1).squeeze(-1)
        new_weight = None
        if not self.opts.two_stage:
            
            for i in range(self.new_classes):
                if new_weight is None:
                    new_weight = jt.nn.matmul(self.weight_new[i].unsqueeze(0), self.weight_old[i] * imprinting_w) 
                else:
                    new_weight = jt.contrib.concat([new_weight, jt.nn.matmul(self.weight_new[i].unsqueeze(0), self.weight_old[i] * imprinting_w)], dim = 0)
            new_bias = self.new_bias
        
        if self.opts.two_stage:
            out.append(jt.nn.conv2d(x_pl, self.new_classifier_weight, self.new_classifier_bias))
        else:
            out.append(jt.nn.conv2d(x_pl, new_weight.unsqueeze(-1).unsqueeze(-1), new_bias))
    ...
    def init_via_weight(self):
        ...
        if self.opts.two_stage:
            ...
            new_weight_bg = self.weight_old_bg * imprinting_w[0] # .squeeze(-1).squeeze(-1) # xzy modified
            new_weight_bg = jt.nn.matmul(self.weight_new_bg, new_weight_bg).unsqueeze(-1).unsqueeze(-1)
            gamma_bg = (imprinting_w[0].norm(p = 2).mean() / new_weight_bg.norm(p = 2).mean())

            # self.cls[0].weight.data[0] = new_weight_bg * gamma_bg #* 0.8
            self.cls[0].weight[0] = (new_weight_bg * gamma_bg).squeeze(0) #[1, 256, 1, 1]
            imprinting_b = jt.contrib.concat([x.bias for x in self.cls[:-1]], dim=0)
    ...
    def execute(self, x, scales=None, do_flip=False, ret_intermediate=False, only_bg=False):
        out_size = x.shape[-2:]
