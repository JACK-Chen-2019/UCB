...
def modify_command_options(opts):
    ...
    if opts.method is not None:
        ...
        if opts.method == "ours":
            ...
            # opts.backbone = 'setr_b' 
            opts.backbone = 'swin_b' 
            opts.two_stage = False 

            opts.fix_pre_cls = False
            opts.warm_up = True
            
            
            opts.warm_epochs = 5# ade: 15 voc: 5

            opts.unce_in_warm = True 
            opts.unkd_in_warm = False 
            opts.unkd_in_warm_factor = 10
            ...
            #     opts.pod_options = {"switch": {"after": {"extra_channels": "sum", "factor": 0.0005, "type": "local"}}}
            #     opts.pseudo = "entropy"
            #     opts.threshold = 0.001 # ori: 0.001
            #     opts.classif_adaptive_factor = True
            #     opts.init_balanced = True
...
def get_argparser():
    ...
    # Performance Options
    parser.add_argument("--local_rank", type=int, default=0) # ade: --local-rank   voc: --local_rank
    parser.add_argument("--random_seed", type=int, default=42, help="random seed (default: 42)")
    parser.add_argument("--num_workers", type=int, default=8, help='number of workers (default: 1)')
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=['FT', 'LWF', 'LWF-MC', 'ILT', 'EWC', 'RW', 'PI', 'MiB', 'PLOP', 'ours'],
    ...
    parser.add_argument(
        ...
        type=str,
        default='resnet101',
        choices=['resnet50', 'resnet101', 'mitb2', 'setr_l', 'setr_b', 'swin_b'],
    ...
    # ==== xzy added ====
    parser.add_argument("--warm_up", action='store_true', default=False, help="Use this to open up warm up strategy")
    parser.add_argument("--align", default=False, action="store_true", help="Use align or not")
    parser.add_argument("--warm_epochs", type=int, default=10, help="warm up epochs number for classifier initialization")
    parser.add_argument("--unce_in_warm", action='store_true', default=False, help="Use UNCE in warm-up")
    parser.add_argument("--unkd_in_warm", action='store_true', default=False, help="Use UNKD in warm-up")
    parser.add_argument("--unkd_in_warm_factor", type=int, default=10, help="factor of unkd in warm")
    parser.add_argument("--two_stage", default=False,help="warm new classifiers by two stages")
    parser.add_argument(
        "--pretrained",
        action='store_true',
        default=True,
    ...
    parser.add_argument("--ucb", default=False, action="store_true")
    return parser
