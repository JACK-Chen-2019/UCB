...
#                         soft_crossentropy)
from utils.loss import UnbiasedCrossEntropy, UnbiasedKnowledgeDistillationLoss 
#===

from entropy_jittor import EntropyWeights
import copy
...
class Trainer:
    ...
    def __init__(self, model, model_old, device, opts, trainer_state=None, classes=None, step=0):
        ...
        if hasattr(self, 'nb_current_classes') and self.nb_current_classes is not None:
            self.entropy_weights = EntropyWeights(self.nb_current_classes, self.old_classes)

        self.opts = opts

    
    def select(self, train_loader, print_int=10, logger=None):
        
        device = self.device
        embedding_dim = self.model_old.head_channels
        new_classes_id = [x + self.old_classes for x in range(self.nb_new_classes)]
        bucket = jt.zeros(self.nb_new_classes, self.old_classes, embedding_dim, dtype=jt.float32).to(device)
        nums = jt.zeros(self.nb_new_classes, dtype = jt.int64).to(device)
        ...
        for cur_step, (images, labels) in enumerate(train_loader):
            ...
            if cur_step > 0:
                print("only run one iter")
                break
            total_class_num = self.old_classes + self.nb_new_classes
            images = images.to(device, dtype=jt.float32)
            labels = labels.to(device, dtype=jt.int64)
            from jittor.transform import Resize, Compose
            import PIL
            # resize=Compose([
            #     Resize((128, 128), interpolation=PIL.Image.NEAREST),
            #  ])
            labels = jt.nn.resize(labels, (128, 128), mode='nearest') 
            ...
            with jt.no_grad():
                ...
                outputs_old = jt.nn.interpolate(outputs_old, size = (128, 128), mode='bilinear', align_corners=False)
                outputs_old = jt.nn.softmax(outputs_old, dim = 1).permute(0, 2, 3, 1).reshape(-1, self.old_classes)
                assert features_old['pre_logits'].shape[1] == embedding_dim
                dim = embedding_dim 
                # upsample = nn.Upsample(scale_factor=2, mode='bilinear')
                pre_feature = features_old['pre_logits'] # upsample(features_old['pre_logits']): will lead to CUDA OOM
                pre_feature = jt.nn.interpolate(pre_feature, size = (128, 128), mode='bilinear', align_corners=False)
                pre_feature = pre_feature.permute(0, 2, 3, 1).reshape(-1, dim)
                imprinting_w = jt.contrib.concat([x.weight for x in self.model_old.cls], dim=0).squeeze(-1).squeeze(-1)
                # for new_class_id in new_classes_id:
                #===xzy modified for speeding up the process of selection
                unique_elements = jt.misc.unique(labels).tolist()
                ...
                #===
                    ...
                    hadamard_product = jt.nn.relu(hadamard_product)
                    hadamard_product = jt.where(hadamard_product > 0, 1, 0)
                    ...
                    if cur_class_score.shape[0] != 0:
                        bucket[new_class_id - self.old_classes] = bucket[new_class_id - self.old_classes] + jt.sum(cur_class_score, dim = 0)
                        nums[new_class_id - self.old_classes] = nums[new_class_id - self.old_classes] + cur_class_score.shape[0]
        ...
        for i in new_classes_id:
            bucket[i - self.old_classes] /= nums[i - self.old_classes]
        return bucket
    def warm_up(self, cur_epoch, optim, train_loader, scheduler=None, print_int=10, logger=None):
        """Warm up and return warm epoch loss"""
        # use pytorch ddp
        # scaler = amp.GradScaler()
        # logger.info("Warm Epoch %d, lr = %f" % (cur_epoch, optim.param_groups[0]['lr']))
        ...
        warm_criterion = criterion
        if self.opts.unce_in_warm:
            warm_criterion = UnbiasedCrossEntropy(
                old_cl=self.old_classes, ignore_index=255, reduction='none'
            )
        ukd_loss = jt.Var(0.)
        if self.opts.unkd_in_warm:
            warm_unkd = UnbiasedKnowledgeDistillationLoss(reduction="none", alpha=self.opts.alpha)
        if hasattr(model, 'weight_old'):
            model.weight_old.requires_grad = True
        if hasattr(model, 'weight_new'):
            model.weight_new.requires_grad = True 
        if hasattr(model, 'new_bias'):
            model.new_bias.requires_grad = True
        if hasattr(model, 'weight_old_bg'):
            model.weight_old_bg.requires_grad = True
        if hasattr(model, 'weight_new_bg'):
            model.weight_new_bg.requires_grad = True
        if hasattr(model, 'new_classifier_weight'):
            model.new_classifier_weight.requires_grad = True
        if hasattr(model, 'new_classifier_bias'):
            model.new_classifier_bias.requires_grad = True

      
    
        for cur_step, (images, labels) in enumerate(train_loader):
            if cur_step > 1:
                print("only run 1 iter...")
                break
            images = images.to(device, dtype=jt.float32)
            labels = labels.to(device, dtype=jt.int64)
            
            ### scaler for mix precision training


            # with amp.autocast(): # pytorch ddp
            if True:
                optim.zero_grad()
                outputs = model(images, ret_intermediate=False)
                
                if isinstance(outputs, tuple):
                    outputs = outputs[0].contiguous()
                else:
                ...
                if self.opts.unkd_in_warm and outputs_old is not None:
                    
                    ukd_loss = warm_unkd(outputs, outputs_old, labels)
                    ukd_loss = jt.mean(ukd_loss)
                    loss_tot = loss_tot + ukd_loss * self.opts.unkd_in_warm_factor
                ...
                #     logger.info(
                #         f"Warm up Epoch {cur_epoch}, Batch {cur_step + 1}/{len(train_loader)},"
        ...
        # warm_epoch_loss = torch.tensor(warm_epoch_loss).to(self.device)
        
        # torch.distributed.reduce(warm_epoch_loss, dst=0)

        # if distributed.get_rank() == 0:
        #     warm_epoch_loss = warm_epoch_loss / distributed.get_world_size() / len(train_loader)
        
        # logger.info(f"Warm Epoch {cur_epoch}, Class Loss={warm_epoch_loss}")

        return warm_epoch_loss
    ...
    def train(self, cur_epoch, optim, train_loader, scheduler=None, print_int=10, logger=None):
        ...
        if self.fix_pre_cls is True:
            for i in range(len(model.cls)):
                if i != len(model.cls) - 1:
                    if not self.use_cosine:
                        for p in model.cls[i].parameters():
                            p.requires_grad = False
                        model.cls[i].eval()
                    elif self.use_cosine:
                        model.cls[i].requires_grad = False
        for cur_step, (images, labels) in enumerate(train_loader):
            ...
            if cur_step > 1:
                ...
                break
            images = images.to(device, dtype=jt.float32)
            labels = labels.to(device, dtype=jt.int8)
            #===
            ...
            if True:
                ...
                if self.step > 0:
                    ...
                    #     probs = jt.softmax(outputs_old, dim=1)
                    ...
                    #     probs_old = jt.softmax(outputs_old, dim=1)
                ...
                optim.zero_grad()
                outputs, features = model(images, ret_intermediate=True)#=self.ret_intermediate)
                ...
                #                                                                     mask_valid_pseudo]
                #         loss_pseudo = jt.nn.cross_entropy(
                ...
                # if self.ucb and self.step > 0 and hasattr(self, 'nb_current_classes'):
                #     sample_weights = self.entropy_weights.get_weight(labels, outputs_old)
                #     loss = loss * sample_weights.flatten()
                ...
                #     mask_new = labels > 0
                #     entropies = entropy(jt.softmax(outputs, dim=1))
                ...
                #===xzy modified
                loss_tot = loss + lkd #+ lde + l_icarl + pod_loss * self.opts.pod_coef + loss_entmin 
                # apex
                # with amp.scale_loss(loss_tot, optim) as scaled_loss:
                #     scaled_loss.backward()
                optim.step(loss)
    ...
    def find_median(self, train_loader, device, logger, mode="probability"):
        ...
        for cur_step, (images, labels) in enumerate(train_loader):
            ...
            mask_bg = labels == 0
            probas = jt.softmax(outputs_old, dim=1)
            ...
            x_coords = pseudo_labels[mask_bg].view(-1) # 这里的pseudo label是朴素的，没有经过阈值筛选
            y_coords = torch.clamp((values_to_bins * nb_bins).long(), max=nb_bins - 1) # 注意这里强制转了long
    ...
    def tensor_to_image(self, one_tensor, id):
        import torchvision.transforms as transforms
        from torchvision.utils import save_image
        from PIL import Image
        mean = torch.tensor([0.485, 0.456, 0.406]).to(one_tensor.device)
        std = torch.tensor([[0.229, 0.224, 0.225]]).to(one_tensor.device)
        tensor = one_tensor[0]
        ...
        # image.save("CAM_viz/" + str(id) + '.jpg')
        return image
    def validate_for_CAM(self, loader, metrics, ret_samples_ids=None, logger=None, end_task=False, class_id=None): # this code is like a shit and will be cleaned
        ...
        if self.step > 0 and self.align_weight_frequency == "epoch":
            model.align_weight(self.align_weight)
        elif self.step > 0 and self.align_weight_frequency == "task" and end_task:
            model.align_weight(self.align_weight)
        ret_samples = []
        # with torch.no_grad(): # if not commented out: RuntimeError: element 0 of tensors does not require grad and does not have a grad_fn
        if True:
        ...
        return (None, None), score, ret_samples
    def validate(self, loader, metrics, ret_samples_ids=None, logger=None, end_task=False):
        ...
        with jt.no_grad():
            for i, (images, labels) in enumerate(loader):
                ...
                #===xzy modified
                prediction, _ = outputs.argmax(dim=1)
...
def features_distillation(
    ...
    for i, (a, b) in enumerate(zip(list_attentions_a, list_attentions_b)):
        ...
        if collapse_channels == "spatial":
            ...
            a = jt.contrib.concat([a_h, a_w], dim=-1)
            b = jt.contrib.concat([b_h, b_w], dim=-1)
            ...
            if pod_deeplab_mask and (
                ...
                if deeplabmask_upscale:
                    ...
                    mask = jt.nn.interpolate(mask[:, None].float(), size=a.shape[-2:]).bool()[:, 0]
        ...
        if normalize:
            a = jt.nn.normalize(a, dim=1, p=2)
            b = jt.nn.normalize(b, dim=1, p=2)
        ...
        if difference_function == "frobenius":
            ...
            layer_loss = jt.nn.kl_div(a, b, reduction="none").view(d1, d2, d3).sum(dim=(1, 2))
    ...
    for scale_index, scale in enumerate(spp_scales):
        ...
        for i in range(scale):
            for j in range(scale):
                ...
                if normalize:
                    horizontal_pool = jt.nn.normalize(horizontal_pool, dim=1, p=2)
                    vertical_pool = jt.nn.normalize(vertical_pool, dim=1, p=2)

                emb.append(horizontal_pool)
                emb.append(vertical_pool)

    return jt.contrib.concat(emb, dim=1)
...
def _global_pod(x, spp_scales=[2, 4, 8], normalize=False):
    ...
    emb = []
    for scale in spp_scales:
        tensor = jt.nn.avg_pool2d(x, kernel_size=w // scale)
        ...
        tensor_pool = jt.contrib.concat([horizontal_pool, vertical_pool], dim=-1)
