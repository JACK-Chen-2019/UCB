...
def resized_crop(
    ...
    img = transform.crop(img, top, left, height, width)
    img = transform.resize(img, size, interpolation)
    return img
...
class Resize(object):
    ...
    def __call__(self, img, lbl=None):
        ...
        if lbl is not None:
            return jt.transform.resize(img, self.size, self.interpolation), jt.transform.resize(lbl, self.size, Image.NEAREST)
        else:
            return jt.transform.resize(img, self.size, self.interpolation)
        #===
...
class CenterCrop(object):
    ...
    def __call__(self, img, lbl=None):
        ...
        if lbl is not None:
            return jt.transform.center_crop(img, self.size), jt.transform.center_crop(lbl, self.size)
        else:
            return jt.transform.center_crop(img, self.size)
        #===
...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img, lbl=None):
        ...
        if random.random() < self.p:
            if lbl is not None:
                return jt.transform.hflip(img), jt.transform.hflip(lbl)
            else:
                return jt.transform.hflip(img)
        #===
        if lbl is not None:
            return img, lbl
        else:
...
class ToTensor(object):
    ...
    def __call__(self, pic, lbl=None):
        ...
        if lbl is not None:
            ...
            return to_tensor(pic), to_tensor(np.array(lbl, dtype=np.uint8))
        else:
...
class Normalize(object):
    ...
    def __call__(self, tensor, lbl=None):
        ...
        img_normalize = transform.ImageNormalize(mean=self.mean, std=self.std)
        if lbl is not None:
            
            return img_normalize(tensor), lbl
        else:
            return img_normalize(tensor)
    ...
    def __call__(self, img, lbl=None):
        ...
        if lbl is not None:
            return resized_crop(img, i, j, h, w, self.size, self.interpolation), \
                   resized_crop(lbl, i, j, h, w, self.size, Image.NEAREST)
        else:
            return resized_crop(img, i, j, h, w, self.size, self.interpolation)
        #===
    def __repr__(self):
