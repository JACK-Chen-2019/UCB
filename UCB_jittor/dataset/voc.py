...
class VOCSegmentationIncremental(Dataset):
#===
        ...
        **kwargs
        ...
        if labels is not None:
            ...
            if idxs_path is not None and os.path.exists(idxs_path):
                ...
                # do not support distributed training for now
            ...
            #     )
            # )
            reorder_transform = jt.transform.Lambda(
                lambda t: t.apply_(
                    lambda x: self.inverted_order[x] if x in self.inverted_order else masking_value
                )
            )
            ...
            if masking:
                ...
                # target_transform = jt.transform.Lambda(
                ...
                # )
                    ...
                    #     t[i] = self.inverted_order[t[i]] if t[i] in tmp_labels else masking_value
                    # return t
                # target_transform = jt.transform.Lambda(self.transform_inplace)
            #===
            else:
                assert False
            ...
            #===xzy modified
            self.dataset = Subset(full_voc, idxs, transform, masking, tmp_labels, masking_value, self.inverted_order)
            #===
        else:
