...
class UnbiasedCrossEntropy(nn.Module):
    ...
    def execute(self, inputs, targets, mask=None):
        ...
        old_cl = self.old_cl
        outputs = jt.zeros_like(inputs)  # B, C (1+V+N), H, W
        den = jt.nn.logsumexp(inputs, dim=1)  # B, H, W       den of softmax
        outputs[:, 0] = jt.nn.logsumexp(inputs[:, 0:old_cl], dim=1) - den  # B, H, W       p(O)
        outputs[:, old_cl:] = inputs[:, old_cl:] - den.unsqueeze(dim=1)  # B, N, H, W    p(N_i)
        ...
        dim = outputs.shape[1]
        outputs = outputs.permute(0, 2, 3, 1).reshape(-1, dim)
        labels = labels.flatten()
        # print(self.ignore_index) 255
        # loss = jt.nn.nll_loss(outputs, labels, ignore_index=self.ignore_index, reduction=self.reduction)
       
        loss = jt.nn.nll_loss(outputs, labels, reduction=self.reduction)
...
class UnbiasedKnowledgeDistillationLoss(nn.Module):
    ...
    def execute(self, inputs, targets, mask=None):
        ...
        new_bkg_idx = jt.misc.to(jt.Var([0] + [x for x in range(targets.shape[1], inputs.shape[1])]), 'cuda')

        den = jt.nn.logsumexp(inputs, dim=1)  # B, H, W
        outputs_no_bgk = inputs[:, 1:-new_cl] - den.unsqueeze(dim=1)  # B, OLD_CL, H, W
        outputs_bkg = jt.nn.logsumexp(
            jt.misc.index_select(inputs, dim=1, indices=new_bkg_idx), dim=1
        ) - den  # B, H, W

        labels = jt.nn.softmax(targets, dim=1)  # B, BKG + OLD_CL, H, W
        ...
        if self.reduction == 'mean':
            outputs = -jt.nn.mean(loss)
        elif self.reduction == 'sum':
            outputs = -jt.nn.sum(loss)
        else:
            outputs = -loss
